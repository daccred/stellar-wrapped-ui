...
export function ActiveLPs({ pools, poolCount }: ActiveLPsProps) {
  return (
    ...
    <StoryHeader title="ACTIVE LPS" chip={` Pools Participating: ${poolCount}`} chipDisplay="chip"/>
        <div className="space-y-4">
          {pools.map((pool, index) => (
            <motion.div
              key={pool.name}
              className="space-y-2 border-b border-black pb-4"
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-sm text-[#0F0F0F]">{pool.name}</span>
              </div>

              <div className="space-y-2">
                <div className="flex justify-start items-start gap-1 flex-col">
                  <span className="text-[#505050] text-xs">Total Staked</span>
                  <div className="flex items-center ga-1">
                    <p className="font-semibold uppercase text-[#0F0F0F]">
                      {pool.totalStaked.amount.toLocaleString()} {pool.totalStaked.token}
                    </p>
                    ...
                    <span className="text-xs text-start text-[#0F0F0F]">~15,400 USDC</span>
                <div className="flex justify-start gap-1 text-sm">
                  {pool.stats.fee && (
                    <div className='rounded-full py-1 px-2 border text-xs'>
                      <span className="text-[#505050]">Fee Tier</span>{' '}
                      <span>{pool.stats.fee}</span>
                    </div>
                  )}
                  <div className='rounded-full py-1 px-2 border text-xs'>
                    <span className="text-[#505050]">APY</span>{' '}
